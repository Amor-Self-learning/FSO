...
const mostBlogs = (blogs) => {
  ...
  return authors.find(author => author.blogs === Math.max(...authors.map(author => author.blogs)));
}
...
const mostLikes = (blogs) => {
  ...
  for (const blog of blogs) {
    const authorIndex = findAuthorIndex(blog.author);
    if(authorIndex === -1) authors.push({author: blog.author, likes: blog.likes});
    else authors[authorIndex].likes += blog.likes;
  }
  return authors.find(author => author.likes === Math.max(...authors.map(author => author.likes)));
}
