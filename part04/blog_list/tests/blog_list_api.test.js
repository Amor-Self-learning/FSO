...
const {blogs, users, usersPasswords} = require('./test_blogs');
...
test('A valid blog can be added', async () => {
  ...
  const resp = await api.post('/api/login').send({username : users[0].user.username, password : users[0].password})
...
test('A blog can be deleted', async () => {
  ...
  const resp = await api.post('/api/login').send({username : blogToDelete.user.username, password : password})
  const token = resp.body.token;
...
test('A blog can be updated', async () => {
  ...
  const resp = await api.post('/api/login').send({username : blogToUpdate.user.username, password : password})
  const token = resp.body.token;
